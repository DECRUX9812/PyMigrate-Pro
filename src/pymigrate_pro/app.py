...
class PyMigrateApp(ctk.CTk):
    def __init__(self):
        ...
        self.logo_label = ctk.CTkLabel(self.sidebar, text="PyMigrate PRO", font=ctk.CTkFont(size=22, weight="bold"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(30, 10))
        
        self.frames = {}
        for F in (WelcomePage, ModeSelectionPage, BatchUserPage, BackupProgressPage, RestorePage):
...
class WelcomePage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        label = ctk.CTkLabel(self, text="PyMigrate Pro", font=ctk.CTkFont(size=28, weight="bold"))
        label.pack(pady=40)
        
        desc = ctk.CTkLabel(self, text="Enterprise User Profile Migration Tool\n\n"
                                     "Suppports: Batch Transfer, Outlook Profiles, Registry Settings", 
                            font=ctk.CTkFont(size=16))
        desc.pack(pady=20)
        
        btn = ctk.CTkButton(self, text="Start Migration", width=200, height=40, font=ctk.CTkFont(size=15),
...
class ModeSelectionPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        backup_btn = ctk.CTkButton(self, text="📤 Batch Backup", width=220, height=60, font=ctk.CTkFont(size=18),
                                   command=lambda: controller.set_mode("backup"))
        backup_btn.pack(pady=20)
        
        restore_btn = ctk.CTkButton(self, text="📥 Batch Restore", width=220, height=60, font=ctk.CTkFont(size=18),
...
class BatchUserPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(left_panel, text="Profiles Found:", font=ctk.CTkFont(weight="bold")).pack(pady=5)
        ...
        ctk.CTkLabel(right_panel, text="Advanced Options:", font=ctk.CTkFont(weight="bold")).pack(pady=5)
        
        self.reg_var = BooleanVar(value=True)
        ctk.CTkCheckBox(right_panel, text="Export Registry (Outlook / Wallpaper)", variable=self.reg_var).pack(anchor="w", pady=10, padx=10)
        
        self.mail_var = BooleanVar(value=True)
        ctk.CTkCheckBox(right_panel, text="Include Mail Profiles (OST/PST)", variable=self.mail_var).pack(anchor="w", pady=10, padx=10)
        
        ctk.CTkLabel(right_panel, text="Note: Registry export only works\nfor the currently logged in user.\nOthers will be skipped.", 
                     text_color="gray", font=("Arial", 12)).pack(anchor="w", padx=10)

        # Footer
        next_btn = ctk.CTkButton(self, text="Next: Backup Selected", command=self.prepare_backup, width=200, height=40)
    ...
    def prepare_backup(self):
        ...
        for user in target_users:
            ...
            folders_to_backup = ["Desktop", "Documents", "Downloads", "Pictures", "Music", "Videos", "Favorites"]
...
class BackupProgressPage(ctk.CTkFrame):
    ...
    def start(self):
        ...
        def run():
            ...
            self.progress.set(1)
            self.btn.configure(state="normal", text="Done (Main Menu)", command=lambda: self.controller.show_frame("WelcomePage"))
...
class RestorePage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.run_btn = ctk.CTkButton(self, text="Start Restore", state="disabled", command=self.run)
    ...
    def run(self):
        ...
        def work():
            s, m = self.controller.backend.restore_batch_backup(self.selected, {}, lambda n, p: self.progress.set(p))
